      ...
      <div className="relative">
        ...
        <div className="space-y-12 relative">
          {timelineSteps.map((step, index) => {
            ...
            return (
              <div key={step.title} className={`flex flex-col md:flex-row items-center ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                <div className="flex sm:pl-0 pl-5 items-center justify-start w-full md:w-auto md:absolute md:left-1/2 md:transform md:-translate-x-1/2">
                  <div className="w-12 h-12 bg-purple-100 rounded-full flex items-center justify-center border-4 border-white">
                    <Icon className="w-6 h-6 text-purple-600" />
                  </div>
                </div>

                <div className={`w-full md:w-1/2 ${isEven ? 'md:pr-12' : 'md:pl-12'} md:mt-0`}>
